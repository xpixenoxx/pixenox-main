...
function SortableFaqItem({ id, faq, onEdit, onDelete, onToggleVisibility }: any) {
...
export default function FaqsPage() {
  ...
  const fetchData = async () => {
    const { data } = await supabase.from('faqs').select('*').order('priority', { ascending: true })
  ...
  const toggleVisibility = async (id: string, isVisible: boolean) => {
    ...
    try {
      const { error } = await supabase.from('faqs').update({ is_visible: isVisible }).eq('id', id)
      if (error) throw error
    } catch (e:any) {
      fetchData()
    }
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const { error } = await supabase.from('faqs').delete().eq('id', deleteData.id)
      if (error) throw error
  ...
  const handleModalSave = async (e: React.FormEvent) => {
     ...
     try {
       ...
       let res;
       if (modalMode === 'create') {
          res = await supabase.from('faqs').insert([payload]).select().single()
       } else {
          res = await supabase.from('faqs').update(payload).eq('id', faqToEdit.id).select().single()
       }

       if (res.error) throw res.error
       toast(`FAQ ${modalMode === 'create' ? 'created' : 'updated'} successfully!`, 'success')
  ...
  return (
    <div className="flex flex-col gap-8 max-w-4xl mx-auto">
      <div className="glass-card">
         <div className="flex items-center justify-between mb-6">
            <div>
               ...
               <p className="text-sm opacity-60">Manage FAQs for the public FAQs page</p>
            </div>
      ...
      <ConfirmModal 
        ...
        title="Delete FAQ?"
        description="This will instantly remove it."
      />
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
            <div className="bg-[#0f0f13] border border-white/10 rounded-2xl max-w-2xl w-full shadow-2xl relative">
               <div className="p-6 border-b border-white/10 flex items-center justify-between">
                  <h2 className="text-xl font-bold">{modalMode === 'create' ? 'Create FAQ' : 'Edit FAQ'}</h2>
               ...
               <form onSubmit={handleModalSave} className="p-6 flex flex-col gap-6">
                  ...
                  <div className="flex justify-end gap-4 mt-2">
                     ...
                     <button type="submit" className="admin-button">Save FAQ</button>
                  </div>
               </form>
            </div>
         </div>
